        ...
        for layer in [1,2]:
            for size in ["Long","Short"]:
                for eta in range(1,9):
                    print(f"Fitting region {region} Layer {layer} Eta {eta} Size {size}")
                    
                    rest = 0 if size == "Long" else 1
                    endcap = "M" if region == -1 else "P" 

                    posi_muons = list(filter(lambda x: f"Eta{eta}" in x in x and f"-{endcap}-" in x and f"L{layer}" in x and int(x.split("/")[-1].split(".txt")[-2][-9:-7]) % 2 == rest, [k for k in all_hist_files if "Posi" in k]))  # and f"L{layer}" and int(x.split("/")[-1].split(".txt")[-2][-9:-7]) % 2 == rest
                    nega_muons = list(filter(lambda x: f"Eta{eta}" in x in x and f"-{endcap}-" in x and f"L{layer}" in x and int(x.split("/")[-1].split(".txt")[-2][-9:-7]) % 2 == rest, [k for k in all_hist_files if "Nega" in k])) 
                    ...
                    FitResult.append (  [region, layer,size,eta,-1,mu_nega[0],mu_nega[1],sigma_nega[0],sigma_nega[1]]  )
                    ...
                    FitResult.append (  [region, layer,size,eta,+1,mu_posi[0],mu_posi[1],sigma_posi[0],sigma_posi[1]]  )
                    axs_fit[0][eta-1].set_xlim(min(AVG_posi-RMS_posi,AVG_nega-RMS_nega),max(AVG_posi+RMS_posi,AVG_nega+RMS_nega))

                fig_fit.tight_layout()
                fig_fit.savefig(f"{folder}/1aFit_Region{region}_Layer{layer}_{size}.png")
                fig_fit.savefig(f"{folder}/1aFit_Region{region}_Layer{layer}_{size}.pdf")
                clear_axes_array(axs_fit)
        
    ### END AND PLOT FIT RESULTS
    pd.DataFrame(FitResult,columns=["Region","Layer","Size","EtaP","Muon Charge","Fit Mean","Fit Mean Error","Fit Sigma","Fit Sigma Error"]).to_csv(f"{folder}/PerEtaFitResults.txt", index=False)
